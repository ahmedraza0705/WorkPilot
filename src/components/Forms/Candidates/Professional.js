        ...
        {({ values, setFieldValue }) => {
          useEffect(() => {
            if (candidate?.id) {
              ...
              // Handle Job Category initialization (check both direct ID and populated object)
              const jobCategoryId =
                candidate?.professional?.jobCategoryId ||
                candidate?.professional?.jobCategory?._id ||
                candidate?.professional?.jobCategory?.id;

              if (jobCategoryId) {
                // Ensure we have the label to display
                const label =
                  candidate?.professional?.jobCategory?.jobCategory ||
                  candidate?.professional?.jobCategoryName ||
                  jobCategory?.find(j => j.id === jobCategoryId || j._id === jobCategoryId)?.jobCategory ||
                  "Selected Category"; // Fallback label if not found immediately

                setJobCat({
                  label: label,
                  value: jobCategoryId,
                });
                // Ensure form field is set with the ID
          ...
          useEffect(() => {
            ...
            // Use functional update to avoid stale closures and ensure proper re-renders
            setCandidate(prev => ({ ...prev, professional: values }));
          }, [values]);
          console.log('---------------------');
            ...
            <Form>
              <div>
                ...
                <Row className="gy-1 pt-75" style={{ marginTop: "10px" }}>
                  ...
                  {/* Resume Upload/Update Section - Only shown when handleResumeChange is passed (e.g. Candidate Profile) */}
                  {handleResumeChange && (
                    <Row className="mt-4">
                      <Col xs="12">
