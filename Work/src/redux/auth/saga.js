...
import { MOCK_USERS, MOCK_TOKEN } from "../../auth/mockData";
...
export function* WATCH_SIGN_IN(action) {
  ...
  try {
    let resp;
    if (action.payload.email.endsWith("@test.com")) {
      const mockUser = MOCK_USERS[action.payload.email];
      if (mockUser) {
        resp = { data: { token: MOCK_TOKEN, user: mockUser } };
      } else {
        resp = { msg: "Invalid Mock User" };
      }
    } else {
      resp = yield loginAPI(action.payload);
    }
...
export function* WATCH_SUPER_ADMIN_SIGN_IN(action) {
  ...
  try {
    ...
    if (action.payload.email.endsWith("@test.com")) {
      const mockUser = MOCK_USERS[action.payload.email];
      if (mockUser && mockUser.role.name === "SuperAdmin") {
        resp = { data: { token: MOCK_TOKEN, user: mockUser } };
      } else {
        resp = { msg: "Invalid SuperAdmin Mock User" };
      }
    } else {
      resp = yield superAdminLoginAPI(action.payload);
    }
